...
export function ResolutionVelocity({ data, className }: ResolutionVelocityProps) {
  ...
  const maxResolved = Math.max(...filteredData.map(d => d.resolved));
  const avgResolved = (filteredData.reduce((sum, d) => sum + d.resolved, 0) / filteredData.length).toFixed(1);
  ...
  const firstHalfAvg = firstHalf.reduce((sum, d) => sum + d.resolved, 0) / firstHalf.length;
  const secondHalfAvg = secondHalf.reduce((sum, d) => sum + d.resolved, 0) / secondHalf.length;
