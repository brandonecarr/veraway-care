-- Fix analytics functions by dropping old versions first
-- Run this in Supabase SQL Editor

-- Drop old functions first to ensure clean replacement
DROP FUNCTION IF EXISTS get_response_time_trends(TIMESTAMPTZ, TIMESTAMPTZ);
DROP FUNCTION IF EXISTS get_resolution_velocity(TIMESTAMPTZ, TIMESTAMPTZ);
DROP FUNCTION IF EXISTS get_clinician_workload();
DROP FUNCTION IF EXISTS get_issue_type_distribution(TIMESTAMPTZ, TIMESTAMPTZ);

-- Now recreate all functions with correct schema

-- 1. Response Time Trends (daily average response time)
CREATE FUNCTION get_response_time_trends(
...
CREATE FUNCTION get_resolution_velocity(
  start_date TIMESTAMPTZ,
  end_date TIMESTAMPTZ
)
RETURNS TABLE (
...
-- 3. Clinician Workload Heatmap (FIXED VERSION - uses user_roles table)
CREATE FUNCTION get_clinician_workload()
RETURNS TABLE (
  user_id UUID,
  name TEXT,
  email TEXT,
...
CREATE FUNCTION get_issue_type_distribution(
  start_date TIMESTAMPTZ,
  end_date TIMESTAMPTZ
)
RETURNS TABLE (
